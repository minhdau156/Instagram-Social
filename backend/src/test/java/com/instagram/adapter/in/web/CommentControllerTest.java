...
class CommentControllerTest {
    ...
    void getComments_returns200OK() throws Exception {
        ...
        when(getCommentsUseCase.getComments(any(GetCommentsUseCase.Query.class))).thenReturn(List.of(comment));
    ...
    @Test
    @WithMockUser(username = "123e4567-e89b-12d3-a456-426614174000")
    void addComment_returns201Created() throws Exception {
        UUID postId = UUID.randomUUID();
    ...
    void addComment_unauthenticated_returns401Or403() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/posts/{id}/comments", postId)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().is3xxRedirection()); // Or isForbidden depending on SecurityConfig. Let's accept isUnauthorized as typically we'd map this, or 403 from spring security
    }
    ...
    @Test
    @WithMockUser(username = "123e4567-e89b-12d3-a456-426614174000")
    void getReplies_returns200OK() throws Exception {
        UUID commentId = UUID.randomUUID();
        UUID userId = UUID.randomUUID();
        Comment comment = Comment.builder()
                .id(UUID.randomUUID())
        ...
        when(getRepliesUseCase.getReplies(any(GetRepliesUseCase.Query.class))).thenReturn(List.of(comment));
        when(getUserUseCase.getUser(any(GetUserUseCase.Query.class))).thenReturn(user);

        mockMvc.perform(get("/api/v1/comments/{id}/replies", commentId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data").isArray())
                .andExpect(jsonPath("$.data[0].content").value("Test Reply"));
    }
}
