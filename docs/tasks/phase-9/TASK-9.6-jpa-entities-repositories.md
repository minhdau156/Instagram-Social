...
- [x] Annotate with `@Entity` and `@Table(name = "reports")`.
- [x] Lombok: `@Getter @Setter @Builder @NoArgsConstructor @AllArgsConstructor`.
- [x] Fields — verify every name against `docs/database/schema.sql`:
...
- [x] `@PrePersist` method: if `createdAt` is null, set it to `OffsetDateTime.now()`.
- [x] `@PrePersist` method: if `status` is null, set it to `ReportStatus.PENDING`.
- [x] Do NOT add `@ManyToOne` joins to `UserJpaEntity` for `reporterId` or `reviewedById` — use raw `UUID` fields to avoid fetching user data unintentionally on every report load.

---

### `ReportJpaRepository.java`

- [x] Extends `JpaRepository<ReportJpaEntity, UUID>`.
- [x] `Page<ReportJpaEntity> findByStatusOrderByCreatedAtAsc(ReportStatus status, Pageable pageable)` — for the admin queue (oldest pending first).
- [x] `Page<ReportJpaEntity> findAllByOrderByCreatedAtDesc(Pageable pageable)` — for the admin dashboard showing all reports.
- [x] `Page<ReportJpaEntity> findByStatusOrderByCreatedAtDesc(ReportStatus status, Pageable pageable)` — for filtered views in the admin panel.
- [x] `long countByStatus(ReportStatus status)` — for the dashboard statistics cards.
- [x] `boolean existsByReporterIdAndEntityId(UUID reporterId, UUID entityId)` — for duplicate-report prevention in the service layer.

---

### `UserBlockId.java`

- [x] A separate `@Embeddable` class representing the composite primary key of `user_blocks`.
- [x] Fields: `UUID blockerId` (maps to `blocker_id`) and `UUID blockedId` (maps to `blocked_id`).
- [x] Must implement `Serializable` — JPA requires embedded IDs to be serializable.
- [x] Must override `equals(Object)` and `hashCode()` correctly — both fields must participate in both methods. An incorrect `equals`/`hashCode` will cause silent duplicate-key issues with JPA's first-level cache.
- [x] Use Lombok `@EqualsAndHashCode` and `@NoArgsConstructor @AllArgsConstructor` — Lombok handles `equals`/`hashCode` correctly for this case.

---

### `UserBlockJpaEntity.java`

- [x] Annotate with `@Entity` and `@Table(name = "user_blocks")`.
- [x] Lombok: `@Getter @Setter @Builder @NoArgsConstructor @AllArgsConstructor`.
- [x] `@EmbeddedId UserBlockId id` — composite PK using the `UserBlockId` class above.
- [x] Do NOT repeat `blockerId` and `blockedId` as separate fields — they live inside the embedded ID. Access them as `id.getBlockerId()` and `id.getBlockedId()`.
- [x] `@Column(name = "created_at", nullable = false, updatable = false) OffsetDateTime createdAt`
- [x] `@PrePersist` method: if `createdAt` is null, set it to `OffsetDateTime.now()`.
...
- [x] Extends `JpaRepository<UserBlockJpaEntity, UserBlockId>`.
- [x] `boolean existsById(UserBlockId id)` — Spring Data derives this from the JPA repository automatically; confirm it works with the composite key.
- [x] `@Query("SELECT CASE WHEN COUNT(b) > 0 THEN TRUE ELSE FALSE END FROM UserBlockJpaEntity b WHERE b.id.blockerId = :blockerId AND b.id.blockedId = :blockedId") boolean existsByBlockerIdAndBlockedId(@Param("blockerId") UUID blockerId, @Param("blockedId") UUID blockedId)` — named for readability; avoids constructing a `UserBlockId` object at the call site.
- [x] `@Modifying @Transactional @Query("DELETE FROM UserBlockJpaEntity b WHERE b.id.blockerId = :blockerId AND b.id.blockedId = :blockedId") void deleteByBlockerIdAndBlockedId(@Param("blockerId") UUID blockerId, @Param("blockedId") UUID blockedId)` — bulk delete without loading the entity.
- [x] `Page<UserBlockJpaEntity> findByIdBlockerIdOrderByCreatedAtDesc(UUID blockerId, Pageable pageable)` — finds all blocks for a given blocker.
- [x] `@Query("SELECT b.id.blockedId FROM UserBlockJpaEntity b WHERE b.id.blockerId = :blockerId") List<UUID> findBlockedIdsByBlockerId(@Param("blockerId") UUID blockerId)` — returns only the IDs for feed/search filtering.
- [x] `@Query("SELECT b.id.blockerId FROM UserBlockJpaEntity b WHERE b.id.blockedId = :blockedId") List<UUID> findBlockerIdsByBlockedId(@Param("blockedId") UUID blockedId)` — returns users who blocked the given user.

---

### `AuditLogJpaEntity.java`

- [x] Annotate with `@Entity` and `@Table(name = "audit_logs")`.
- [x] Lombok: `@Getter @Setter @Builder @NoArgsConstructor @AllArgsConstructor`.
- [x] Fields — match the schema exactly:
  ...
  - `@Column(name = "created_at", nullable = false, updatable = false) OffsetDateTime createdAt`
- [x] `@PrePersist`: set `createdAt = OffsetDateTime.now()` if null.
- [x] No business methods needed. Audit log entities are write-once.

---

### `AuditLogJpaRepository.java`

- [x] Extends `JpaRepository<AuditLogJpaEntity, Long>`.
- [x] `Page<AuditLogJpaEntity> findByUserIdOrderByCreatedAtDesc(UUID userId, Pageable pageable)` — for viewing a user's audit history.
- [x] `Page<AuditLogJpaEntity> findByActionOrderByCreatedAtDesc(String action, Pageable pageable)` — for filtering by action type.
- [x] No other custom query methods are needed for Phase 9.
