...
- [x] Method: `Report reportContent(Command command)`
- [x] Inner record `Command` with the following fields:
...
- [x] The method returns the newly created `Report` domain object (with its generated `id` populated by the persistence adapter).

#### `BlockUserUseCase.java`

- [x] Method: `UserBlock blockUser(Command command)`
- [x] Inner record `Command` with the following fields:
...
- [x] The method returns the newly created `UserBlock`.

#### `UnblockUserUseCase.java`

- [x] Method: `void unblockUser(Command command)`
- [x] Inner record `Command` with the following fields:
...
- [x] The method returns `void`. There is no meaningful return value after deletion.

#### `GetBlockedUsersUseCase.java`

- [x] Method: `List<UserBlock> getBlockedUsers(Query query)`
- [x] Inner record `Query` with the following fields:
...
- [x] The method returns a paginated list of `UserBlock` records. The controller (TASK-9.9) enriches each `UserBlock` with user profile info by calling `UserRepository` to look up the blocked user's username and avatar.
...
- [x] Method: `Report reviewReport(Command command)`
- [x] Inner record `Command` with the following fields:
...
- [x] The method returns the updated `Report` domain object reflecting the new status.

#### `SuspendUserUseCase.java`

- [x] Method: `User suspendUser(Command command)`
- [x] Inner record `Command` with the following fields:
...
- [x] The method returns the updated `User` domain object with `status = SUSPENDED`.
- [x] Note: `account_status` is a PostgreSQL ENUM with values `active`, `suspended`, `deactivated`, `pending_verification`. The `User` domain model already has a `UserStatus` (or `AccountStatus`) field that maps to this column. Verify the field name and type in `User.java` before referencing it.

#### `UnsuspendUserUseCase.java`

- [x] Method: `User unsuspendUser(Command command)`
- [x] Inner record `Command` with the following fields:
  - `UUID adminId` — the admin performing the action.
  - `UUID targetUserId` — the user being reinstated.
- [x] The method returns the updated `User` domain object with `status = ACTIVE`.

#### `AdminGetReportsUseCase.java`

- [x] Method: `List<Report> getReports(Query query)`
- [x] Inner record `Query` with the following fields:
  - `ReportStatus status` — filter by status; may be `null` to return all reports.
  - `int page` — zero-based page number.
  - `int size` — page size.
- [x] The method returns a filtered, paginated list of reports ordered by `created_at DESC`.
