...
- [x] `useSubmitReport.ts` — wraps `useMutation` calling `moderationApi.submitReport(payload)`. On success, show a success toast or snackbar ("Report submitted. Thank you."). On error, show an error toast.
- [x] `useBlockUser.ts` — wraps `useMutation` calling `moderationApi.blockUser(username)`. On success, invalidate the user profile query (so the `isBlocked` state refreshes) and show a toast. On error, show an error toast.
- [x] `useUnblockUser.ts` — wraps `useMutation` calling `moderationApi.unblockUser(username)`. On success, invalidate the blocked users list query (`['blocked-users']`) and the user profile query. On error, show an error toast.
- [x] Place all three hooks in `frontend/src/hooks/moderation/` following the `hooks/notification/` sub-folder pattern.
...
- [x] `open: boolean` — controls whether the dialog is visible.
- [x] `onClose: () => void` — called when the user cancels or after successful submission.
- [x] `entityType: ReportEntityType` — the type of content being reported.
- [x] `entityId: string` — the UUID of the content being reported.
- [x] `title?: string` — optional context label shown in the dialog subtitle (e.g., `"Report this post"`, `"Report @username"`). If not provided, derive a generic title from `entityType`.

#### Internal state

- [x] `selectedReason: ReportReason | null` — which radio option is selected; `null` on open.
- [x] `details: string` — the optional free-text field; empty string on open.

#### Hooks to call inside the component

- [x] `const { mutate: submitReport, isPending, isError } = useSubmitReport()`.

#### Layout structure

- [x] Root: MUI `Dialog` with `open={open}` and `onClose={onClose}`.
- [x] `DialogTitle`: display the context title (from `title` prop or derived from `entityType`). Add a close `IconButton` in the top-right corner (`CloseIcon`) that calls `onClose`.
- [x] `DialogContent`:
...
- [x] Error state: if `isError`, show a `Typography color="error"` message beneath the `RadioGroup` saying "Something went wrong. Please try again."
- [x] Reset state on close: use a `useEffect` that watches `open`. When `open` transitions from `true` to `false`, reset `selectedReason` to `null` and `details` to `''`.

#### Where `ReportDialog` is triggered

- [x] Document (without implementing here) that `ReportDialog` should be wired into:
...
- [x] These wiring points are out of scope for this task but must be listed here as follow-up work.
...
- [x] `username: string` — the username of the user to block or unblock.
- [x] `isBlocked: boolean` — whether the current user has already blocked this user. The parent component (profile kebab menu) is responsible for determining this value and passing it in.
- [x] `onToggle?: () => void` — optional callback called after a successful block or unblock so the parent can update its own state.

#### Internal state

- [x] `confirmOpen: boolean` — controls whether the confirmation dialog is shown; `false` by default.

#### Hooks to call inside the component

- [x] `const { mutate: block, isPending: isBlocking } = useBlockUser()`.
- [x] `const { mutate: unblock, isPending: isUnblocking } = useUnblockUser()`.
- [x] `const isPending = isBlocking || isUnblocking`.

#### Layout structure

- [x] If `isBlocked` is `false` (not yet blocked):
...
- [x] If `isBlocked` is `true` (already blocked):
...
- [x] Confirmation dialog (for block only):
...
- [x] The block/unblock button must have an `aria-label` attribute set to `"Block @{username}"` or `"Unblock @{username}"` as appropriate, for screen-reader accessibility.
