...
- [x] Open `docs/database/schema.sql` and locate the `search_history` table before writing any code.
...
- [x] Hand-written `Builder` inner class with all four fields.
- [x] Static `builder()` factory method that returns a new `Builder`.
- [x] `Builder.build()` returns a new `SearchHistory` instance.
- [x] No setters — the object is immutable after construction.
- [x] No business-behaviour methods needed (SearchHistory is a value record, not an entity with behaviour).
- [x] All fields accessible via plain getter methods (`getId()`, `getUserId()`, `getQuery()`, `getSearchedAt()`).
