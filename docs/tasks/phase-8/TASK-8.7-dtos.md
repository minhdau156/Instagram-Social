...
- [x] Record fields:
  ...
  - `boolean isPrivate`
  - `int followerCount`
- [x] `public static UserSearchResponse from(User user)` factory method mapping all fields.
- [x] Note: do NOT include sensitive fields (`email`, `passwordHash`, etc.). Only include fields the frontend needs to render a search result row (avatar, username, display name, follower count, privacy flag).
...
- [x] `public static HashtagSearchResponse from(Hashtag hashtag)` factory method.
...
- [x] `public static PostSearchResponse from(Post post)` factory method.
...
- [x] `public static SearchHistoryResponse from(SearchHistory entry)` factory method.
- [x] Note: there is no `searchType` field — the schema does not store it (see TASK-8.1 schema verification note).
